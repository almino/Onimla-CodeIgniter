...
class Element extends Node implements HasAttribute, Appendable {
    ...
    public function addClass($class) {
        ...
        /*
          # Quebra as classes existentes num array
          $existing = preg_split('/\s+/', $this->attr('class'), PREG_SPLIT_NO_EMPTY);

          /*
          # DEBUG
          echo "\n<code>{$this->name}'s existing CSS classes:</code>\n";
          echo "\n<pre>";
          var_dump($existing);
          echo "</pre>\n";
         */
        /*
          ...
          # Quebra as classes enviadas num único array
          $new = preg_split('/\s+/', implode(' ', self::arrayFlatten(func_get_args())), PREG_SPLIT_NO_EMPTY);

          /*
          # DEBUG
          echo "\n<code>{$this->name}'s new CSS classes:</code>\n";
          echo "\n<pre>";
          var_dump($new);
          echo "</pre>\n";
         */
        ...
        /*
          # Garante que não há posições vazias
          $existing = array_filter($existing, 'strlen');
          $new = array_filter($new, 'strlen');

          # Remove todas as classes duplicadas e junta tudo num único array
          $classes = array_unique(array_merge($existing, $new));

          # Se não houver classes CSS a adicionar
          if (count($classes) < 1) {
          # Remove o atributo
          ...
          if (count($new) > 0) {
          self::log("Adding  \"." . implode('.', $new) . "\" to {$this->path()}");
          } else {
          self::log("No class added to {$this->path()}");
          }

          # Junta as classes numa string separadas por espaços
